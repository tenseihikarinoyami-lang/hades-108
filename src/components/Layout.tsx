...
export const Layout: React.FC = () => {
  ...
  const NavGroup = ({ title, children, mobile = false }: { title: string, children: React.ReactNode, mobile?: boolean }) => (
    <div className={`${mobile ? 'block' : 'relative group/nav'}`}>
      <button className={`flex items-center gap-1 transition-all hover:text-accent hover:neon-text-accent uppercase tracking-widest text-muted-foreground text-[10px] font-bold ${mobile ? 'w-full py-2 justify-start' : ''}`}>
        {title} <ChevronDown className="w-3 h-3" />
      </button>
      <div className={`${mobile
        ? 'ml-4 mt-1 space-y-1'
        : 'absolute top-full left-0 mt-2 w-48 bg-background/95 border border-accent/20 glass-panel opacity-0 invisible group-hover/nav:opacity-100 group-hover/nav:visible transition-all duration-300 z-50 p-2 space-y-1 shadow-[0_10px_30px_rgba(0,0,0,0.5)]'
        }`}>
        {children}
      </div>
    </div>
  ...
  const NavLink = ({ to, icon: Icon, label, color = "hover:text-accent", mobile = false }: { to: string, icon: any, label: string, color?: string, mobile?: boolean }) => (
    <Link
      to={to}
      className={`flex items-center gap-3 px-3 py-2 rounded-sm text-[10px] uppercase tracking-widest text-muted-foreground transition-all ${color} hover:bg-accent/5 ${mobile ? 'w-full' : ''}`}
      onClick={() => { audio.playSFX('click'); if (mobile) setMobileMenuOpen(false); }}
    >
        ...
        <div className="container flex h-20 items-center justify-between px-4 relative">
          ...
          {loading ? (
            ...
            <>
              {/* Mobile Menu Toggle */}
              <button
                className="lg:hidden flex items-center gap-2 text-accent hover:text-accent/80 transition-colors z-50"
                onClick={() => { audio.playSFX('click'); setMobileMenuOpen(!mobileMenuOpen); }}
              >
              ...
              <div className="hidden lg:flex items-center gap-4 sm:gap-6">
                {/* Cataclysm Display */}
                {cataclysm && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-accent/5 border border-accent/20 clip-diagonal animate-pulse">
              ...
              {mobileMenuOpen && (
                <div className="lg:hidden fixed inset-0 top-20 bg-background/95 backdrop-blur-lg z-40 overflow-y-auto">
                  <div className="container px-4 py-6 space-y-6">
                    {/* Cataclysm Display */}
                    {cataclysm && (
                      <div className="flex items-center justify-center gap-2 px-3 py-2 bg-accent/5 border border-accent/20 clip-diagonal animate-pulse max-w-xs mx-auto">
                    ...
                    <div className="space-y-4">
                      <Link to="/" className="flex items-center gap-3 py-2 text-muted-foreground hover:text-accent transition-colors uppercase tracking-widest text-[10px] font-bold" onClick={() => setMobileMenuOpen(false)}>
                        <Home className="w-4 h-4" /> Inicio
                      </Link>

                      <div className="space-y-2">
                        <h3 className="text-xs font-mono text-accent uppercase tracking-widest border-b border-accent/20 pb-2">Aventura</h3>
                        <div className="space-y-1 ml-2">
                          <NavLink to="/trivia" icon={Gamepad2} label="Arena" mobile />
                          <NavLink to="/tower" icon={Castle} label="Torre" color="hover:text-purple-400" mobile />
                          <NavLink to="/campaign" icon={Map} label="Descenso" color="hover:text-purple-500" mobile />
                          <NavLink to="/fishing" icon={Anchor} label="Pesca" color="hover:text-blue-400" mobile />
                          <NavLink to="/saint-mode" icon={BookOpen} label="Leyenda" color="hover:text-yellow-500" mobile />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <h3 className="text-xs font-mono text-accent uppercase tracking-widest border-b border-accent/20 pb-2">End-Game</h3>
                        <div className="space-y-1 ml-2">
                          <NavLink to="/labyrinth" icon={Skull} label="Laberinto" color="hover:text-red-500" mobile />
                          <NavLink to="/battle-royale" icon={Trophy} label="Torneo" color="hover:text-yellow-400" mobile />
                          <NavLink to="/secret-bosses" icon={EyeOff} label="Primordiales" color="hover:text-purple-500" mobile />
                          <NavLink to="/world-boss" icon={Skull} label="Jefe Mundo" color="hover:text-red-600" mobile />
                          <NavLink to="/raids" icon={Swords} label="Incursión" color="hover:text-red-500" mobile />
                          <NavLink to="/auction" icon={StoreIcon} label="Subasta" color="hover:text-yellow-500" mobile />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <h3 className="text-xs font-mono text-accent uppercase tracking-widest border-b border-accent/20 pb-2">Social</h3>
                        <div className="space-y-1 ml-2">
                          <NavLink to="/chat" icon={MessageSquare} label="Cocytos" mobile />
                          <NavLink to="/faction-base" icon={Castle} label="Base" color="hover:text-accent" mobile />
                          <NavLink to="/guilds" icon={Users} label="Escuadrones" color="hover:text-emerald-500" mobile />
                          <NavLink to="/territories" icon={Map} label="Territorios" color="hover:text-orange-500" mobile />
                          <NavLink to="/holy-war" icon={Swords} label="Guerra" mobile />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <h3 className="text-xs font-mono text-accent uppercase tracking-widest border-b border-accent/20 pb-2">Progreso</h3>
                        <div className="space-y-1 ml-2">
                          <NavLink to="/cosmos" icon={Star} label="Cosmos" color="hover:text-pink-400" mobile />
                          <NavLink to="/pets" icon={Dog} label="Familiares" color="hover:text-orange-400" mobile />
                          <NavLink to="/alchemy" icon={FlaskConical} label="Alquimia" color="hover:text-green-400" mobile />
                          <NavLink to="/battle-pass" icon={Crown} label="Pase" color="hover:text-cyan-400" mobile />
                          <NavLink to="/ascension" icon={ArrowUpCircle} label="Ascensión" color="hover:text-blue-400" mobile />
                          <NavLink to="/leaderboard" icon={Trophy} label="Ranking" mobile />
                        </div>
                      </div>

                      <div className="space-y-2">
                        <h3 className="text-xs font-mono text-accent uppercase tracking-widest border-b border-accent/20 pb-2">Equipo</h3>
                        <div className="space-y-1 ml-2">
                          <NavLink to="/equipment" icon={Shield} label="Armería" mobile />
                          <NavLink to="/forge" icon={Hammer} label="Forja" color="hover:text-orange-500" mobile />
                          <NavLink to="/store" icon={StoreIcon} label="Tienda" mobile />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 pt-4 border-t border-accent/20">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-muted-foreground uppercase tracking-widest">Audio</span>
                        <Button variant="ghost" size="icon" onClick={toggleAudio} className="text-muted-foreground hover:text-accent hover:bg-accent/10 rounded-sm">
                          {isMuted ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}
                        </Button>
                      </div>
                      <Link to="/profile" className="flex items-center gap-3 py-2 hover:opacity-80 transition-opacity" onClick={() => setMobileMenuOpen(false)}>
                        <Avatar className="h-10 w-10 border-2 rounded-sm clip-diagonal border-accent/50">
                          <AvatarImage src={profile?.photoURL || user.photoURL || ''} alt={profile?.specterName || 'User'} />
                          <AvatarFallback className="bg-secondary text-accent font-display">
                            {profile?.specterName?.[0] || user.displayName?.[0] || 'U'}
                          </AvatarFallback>
                      ...
                      <Button onClick={handleLogout} variant="outline" className="w-full border-primary/50 text-primary hover:bg-primary/10 clip-diagonal">
